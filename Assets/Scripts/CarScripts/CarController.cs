...
public class CarController : CarComponent
{
	...
	public bool CanDrive => !IsSpinout && !IsBumped && !IsBackfire;
	...
	private static void OnIsBackfireChangedCallback(Changed<CarController> changed) =>
		changed.Behaviour.OnBackfiredChanged?.Invoke(changed.Behaviour.IsBackfire);

	private static void OnIsBumpedChangedCallback(Changed<CarController> changed) =>
		changed.Behaviour.OnBumpedChanged?.Invoke(changed.Behaviour.IsBumped);

	private static void OnSpinoutChangedCallback(Changed<CarController> changed) =>
		changed.Behaviour.OnSpinoutChanged?.Invoke(changed.Behaviour.IsSpinout);

	private static void OnDriftTierIndexChangedCallback(Changed<CarController> changed) =>
		changed.Behaviour.OnDriftTierIndexChanged?.Invoke(changed.Behaviour.DriftTierIndex);

	private static void OnBoostTierIndexChangedCallback(Changed<CarController> changed) =>
	...
	private void Update()
	{
		...
		{
			if (Car.Input.gamepad != null)
			{
				Car.Input.gamepad.SetMotorSpeeds(IsOffroad ? AppliedSpeed / MaxSpeed : 0, 0);
			}
		}
	}
	...
	private void OnCollisionStay(Collision collision)
	{
		...
		if (layer == GameManager.KartLayer && collision.gameObject.TryGetComponent(out CarEntity otherKart))
		{
			//
	...
	{
		if (Track.Current != null && Track.Current.StartRaceTimer.Expired(Runner))
		{
	...
	private void SpinOut(CarInput.NetworkInputData input)
	{
		var isAccelerate = input.IsDown(CarInput.NetworkInputData.ButtonAccelerate);
	...
	private void Drift(CarInput.NetworkInputData input)
	{
	...
	/// </summary>
	/// <param name="input"></param>
	private void Boost(CarInput.NetworkInputData input)
	{
		if (BoostTime > 0)
		{
	...
	private void GroundNormalRotation()
	{
		...
		IsGrounded = Physics.BoxCast(collider.transform.TransformPoint(collider.center), new Vector3(0.48f, 0.48f, 1), Vector3.down, out var hit);
	...
	// One-Shot Functions
	private void StartDrifting(CarInput.NetworkInputData input)
	{
