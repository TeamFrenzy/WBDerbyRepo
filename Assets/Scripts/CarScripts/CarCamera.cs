...
public class CarCamera : CarComponent, ICameraController
{
	...
	{
		var lookBehindThisFrame = _lastFrameLookBehind != Car.Input.IsLookBehindPressed;

		rig.localEulerAngles = _viewpoint.localEulerAngles;

		_lastFrameLookBehind = Car.Input.IsLookBehindPressed;
	...
	{
		_currentFOV = useFinishVP ? finishFOV : Car.Controller.BoostTime > 0 ? boostFOV : normalFOV;
		cam.fieldOfView = Mathf.Lerp(cam.fieldOfView, _currentFOV, Time.deltaTime * lerpFactorFOV);
	}

	private Transform GetViewpoint()
	{
		if (Car.Controller == null) return null;
		if (useFinishVP) return finishVP;

		if (Car.Input.IsLookBehindPressed)
		{
			return Car.Controller.BoostTime > 0 ? backwardBoostVP : backwardVP;
		}

		return Car.Controller.BoostTime > 0 ? forwardBoostVP : forwardVP;
	}
}